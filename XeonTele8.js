...
module.exports = XeonBotInc = async (XeonBotInc, bot) => {
    ...
    try {
        ...
        const username = XeonBotInc.message.from.username ? XeonBotInc.message.from.username : "DEVILXKING06";
...
break;
    ...
    /*if (!Array.isArray(adminIDs) || !adminIDs.includes(userID)) {
        return XeonBotInc.telegram.sendMessage(
            ...
            `📩 *Send proof to the owner @DEVILXKING06*`,
            {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "📢 Telegram Channel", url: "https://t.me/devilCrahVip" }],
    ...
    return XeonBotInc.telegram.sendMessage(
        XeonBotInc.chat.id, 
        `🚫 *You are not authorized to use this command\\.@DEVILXKING06*\n\n` +
        `📩 *Please contact the developer to buy:* \n\n` +
        `💰 *Price/Harga:*\n` +
        `✅ *Access permanent:* 10$\n` +
        `✅ *Resell permanent:* 25$\n` +
        `✅ *Script no enc 100%:* 80$`,
        {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "📢 Telegram Group", url: "https://t.me/+CiQitI3yc4QyMzQ1" }],
                    [{ text: "▶️ YouTube Channel", url: "https://youtube.com/@DEVIL-KING-STORE" }],
                    [{ text: "📱 WhatsApp Channel", url: "https://whatsapp.com/channel/0029Vb2zlJP1CYoLCdr1k13f" }]
                ]
            },
...
case 'runtime':{
    XeonBotInc.deleteMessage().catch(() => {});
      reply(`devil crash v6 is Online ${runtime(process.uptime())}`)
    }
  break
case 'menu': case 'back!':
